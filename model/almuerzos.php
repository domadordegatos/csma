<?php
    ...
    class edicion_usuario{
            ...
            $pago_final_2 = $valor_antiguo - $pago_final;
                    ...
                    if($ejecutar){
                        for ($i=0; $i < count($datos) ; $i++) {
                            ...
                            $insert="INSERT INTO movimientos_almuerzos VALUES ('','$id_factura','$id_admin','$id_user',3,$d[0],'$c','$valor_antiguo','$pago_final_2','$fecha','$hora',4,0)";
        ...
        function crearventa(){
            ...
            $pre_do=self::almuerzo_previo(4,$id_user);
            $pre_sa=self::almuerzo_previo(5,$id_user);
            if($total_pe > 0){if($total_pe > $pre_pe){ return 7;}}
            if($total_gr > 0){if($total_gr > $pre_gr){ return 7;}}
            if($total_do > 0){if($total_do > $pre_do){ return 7;}}
            if($total_sa > 0){if($total_sa > $pre_sa){ return 7;}}
                    ...
                    if($ejecutar){
                        for ($i=0; $i < count($datos) ; $i++) {
                            ...
                            $insert="INSERT INTO movimientos_almuerzos VALUES ('','$id_factura','$id_admin','$id_user',1,$d[0],'$c','$valor_antiguo','$valor_antiguo','$fecha','$hora',4,0)";
        ...
        function recargar(){
            ...
            require_once "conexion.php";
            $conexion=conexion();
            $pequeno=$_POST['form1']; $grande=$_POST['form2']; $docente=$_POST['form3']; $saludable=$_POST['form4']; $id=$_POST['form5'];$valor_almuerzos=$_POST['form6']; $medio_pago=$_POST['form7'];
            $id_factura=self::crearfolio();
            $id_usuario=self::id_usuario($id);
            ...
            if(mysqli_num_rows($ejecutar)<=0){ /* no hay registro del usuario */
                ...
                if($pequeno<0 || $grande<0 || $docente<0 || $saludable<0){/* estas tratando de ingresar un valor nulo */
                    ...
                    $pe=self::valores_almuerzos(21);
                    $gr=self::valores_almuerzos(22);
                    $do=self::valores_almuerzos(23);
                    $sa=self::valores_almuerzos(24);
                    ...
                    if($pequeno == 0 && $grande == 0 && $docente == 0 && $saludable == 0){
                        ...
                        $insert="INSERT INTO movimientos_almuerzos VALUES ('','$id_factura','$id_admin','$id_usuario','2','5','$cantidad','$val_antiguo','$val_antiguo','$fecha','$hora','$medio_pago','$valor_almuerzos')";
            ...
            if(mysqli_num_rows($ejecutar)<=0){ /* no hay registro del usuario */
                ...
                if($valor<=0){/* estas tratando de ingresar un valor nulo */
                    ...
                    if($valor>$ver[0]){
                        ...
                        $insert="INSERT INTO movimientos_almuerzos VALUES ('','$id_factura','$id_admin','$id_usuario',4,5,0,'$ver[0]','$nuevo_saldo','$fecha','$hora','$medio_pago','$valor')";
                        $ejecutar2=mysqli_query($conexion, $insert);
        ...
        function pagar_deudas(){
            ...
            $id_admin=self::id_admin($user);
                    ...
                    $ejecutar=mysqli_query($conexion, $sql);
                    if($ejecutar){
                        $insert="INSERT INTO movimientos_almuerzos VALUES ('','$id_factura','$id_admin','$id_usuario',5,5,0,'$valor_antiguo','$valor_antiguo','$fecha','$hora',4,0)";
                        $ejecutar2=mysqli_query($conexion, $insert);
